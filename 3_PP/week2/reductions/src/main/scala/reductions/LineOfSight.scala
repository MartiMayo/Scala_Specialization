...
object LineOfSight {
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    var max: Float = 0
    if (input.length > 0) output(0) = input(0)
  ...
  def upsweep(input: Array[Float], from: Int, end: Int,
    threshold: Int): Tree = {
      if (end - from <= threshold) Leaf(from, end, upsweepSequential(input, from, end))
        ...
        val m = (from + end) / 2
        val (tl, tr) = parallel(upsweep(input, from, m, threshold), upsweep(input, m, end, threshold))
        Node(tl, tr)
      }
  }
  ...
  def downsweep(input: Array[Float], output: Array[Float], startingAngle: Float,
    ...
    case Node(l, r) => {
      parallel(downsweep(input, output, startingAngle, l), downsweep(input, output, Math.max(l.maxPrevious, startingAngle), r))
    }
  }
